                ...
                <tbody>
                    ...
                    <tr class="border-top">
                        <td class="ps-4 py-3 text-nowrap">
                            <div class="fw-bold text-dark">{{ \Carbon\Carbon::parse($rdv->date)->format('d M Y') }}</div>
                            <div class="text-muted small"><i class="bi bi-clock me-1"></i>{{ $rdv->time }}</div>
                        </td>
                        ...
                        <td class="text-center text-nowrap">
                            <span class="doctor-badge">
                                <i class="bi bi-person-fill"></i> Dr. {{ $rdv->doctor->name ?? 'N/A' }}
                            </span>
                        </td>
                        ...
                        <td class="text-end pe-4 text-nowrap">
                            <div class="d-flex justify-content-end gap-1">
                                <button class="btn-icon view" data-bs-toggle="modal" data-bs-target="#viewModal{{ $rdv->id }}">
                                    <i class="bi bi-eye"></i>
                                </button>
                                  <form action="{{ route('admin.appointments.destroy', $rdv->id) }}" method="POST" id="delete-form-{{ $rdv->id }}" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn-icon delete" onclick="confirmDelete({{ $rdv->id }})">
                    ...
                    <div class="modal fade" id="viewModal{{ $rdv->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content border-0 shadow" style="border-radius: 15px;">
                                <div class="modal-header border-0 bg-light" style="border-radius: 15px 15px 0 0;">
                                    <h5 class="modal-title fw-bold text-dark"><i class="bi bi-info-circle me-2 text-emerald"></i>Fiche RDV #{{ $rdv->id }}</h5>
                                ...
                                <div class="modal-body p-4">
                                    ...
                                    <div class="row g-4">
                                        <div class="col-6">
                                            ...
                                            <span class="fw-bold text-dark">{{ $rdv->patient_name }}</span>
                                        </div>
                                        <div class="col-6 text-end">
                                            <label class="small text-muted d-block mb-1 text-uppercase fw-bold" style="font-size: 0.65rem;">Téléphone</label>
                                            <span class="fw-bold text-dark text-nowrap"><i class="bi bi-telephone me-1 text-emerald"></i> {{ $rdv->patient_phone }}</span>
                                        </div>
                                        ...
                                        <div class="col-6 border-top pt-3">
                                            ...
                                            <span class="fw-bold text-dark"><i class="bi bi-calendar-event me-2 text-emerald"></i>{{ \Carbon\Carbon::parse($rdv->date)->format('d M Y') }}</span>
                                        </div>
                                        <div class="col-6 text-end border-top pt-3">
                                            ...
                                            <span class="fw-bold text-dark"><i class="bi bi-clock me-2 text-emerald"></i>{{ $rdv->time }}</span>
                                        </div>
                                        ...
                                        <div class="col-12 border-top pt-3">
                                            ...
                                            <div class="p-3 bg-light rounded-3 border-start border-emerald border-3 text-secondary shadow-sm">
                                                <i>" {{ $rdv->reason ?? 'Aucun motif renseigné' }} "</i>
                                            </div>
                                        </div>

                                        <div class="col-12 text-center pt-2">
                                            <span class="badge-custom {{ $badgeClass }}">{{ $label }}</span>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="card-footer bg-white border-0 py-4">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div class="text-muted small">
                    Affichage de <b>{{ $appointments->firstItem() }}</b> à <b>{{ $appointments->lastItem() }}</b> sur <b>{{ $appointments->total() }}</b>
                </div>
